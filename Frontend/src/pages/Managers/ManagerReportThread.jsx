                ...
                <div className="flex items-start gap-3 mb-4">
                    ...
                    <div className="flex-1">
                        ...
                        <p className="text-sm text-gray-500">
                            Posted by {report.author?.name || 'Staff'} • {formatDate(report.createdAt)}
                        </p>
                    </div>
                </div>
                <div className="pl-11">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 mb-6">
                ...
                {report.replies && report.replies.length > 0 ? (
                    <div className="space-y-4 mb-6">
                        {report.replies.map((reply, index) => (
                            <div
                                key={index}
                                className={`flex gap-3 ${reply.authorType === 'manager' ? 'flex-row-reverse' : 'flex-row'}`}
                            >
                                <div className={`p-2 rounded-full ${reply.authorType === 'manager' ? 'bg-blue-100' : 'bg-green-100'}`}>
                                    <UserIcon className={`w-4 h-4 ${reply.authorType === 'manager' ? 'text-blue-600' : 'text-green-600'}`} />
                                </div>
                                <div className={`flex-1 ${reply.authorType === 'manager' ? 'text-right' : 'text-left'}`}>
                                    <div className={`inline-block max-w-[80%] p-3 rounded-lg ${reply.authorType === 'manager'
                ...
                <form onSubmit={handleSubmitReply} className="border-t pt-4">
                    ...
                    <textarea
                        ...
                        className="w-full px-4 text-black py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent resize-none"
                        rows="4"
...
export default ManagerReportThread;
